...
the route specifications own, and what sits outside Turbo-Collection entirely. Note the symmetry: a port on
each end, and nothing vendor-specific in the middle.

```mermaid
flowchart LR
    IN["Sources<br/>iPhone, iCloud, OneDrive,<br/>SD card, existing archive<br/>(specs/acquisition-routes/)"]
...
| ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| Concrete source adapters (iPhone, iCloud, OneDrive, SD card) and their format quirks | Route specifications under `specs/acquisition-routes/`, one per acquisition route |
...
- **Layout convention.** The rule that determines where in the collection a file is stored, given
  that file's own bytes, the metadata its acquisition route supplied with it, and that route.
...
- **Acquisition route.** One way of getting original bytes into the collection, such as iCloud or a
  camera card. A Source adapter implements exactly one acquisition route; configuration names the
  route (R-SRC-3), and R-SRC-10 admits that name into a collection path. What may be assumed about
  one route is stated in its own specification under `specs/acquisition-routes/`.
...
| **R-SRC-10** | The collection path of an item MUST be a pure function of the item's own bytes, the metadata the acquisition route supplies with it, and that route, under the collection's layout convention.                                                                                                                                                                                 |
...
> **What R-SRC-10 still forbids, now that the route is admitted.** This requirement once said the
> collection layout MUST NOT depend on which source supplied a file, so that two identical photos
> arriving by different routes landed in the same place. That guarantee is withdrawn deliberately.
> The acquisition route is a path segment because it is the one thing about an arrival that a file
> cannot state about itself, and a photo acquired by two routes is therefore two files; R-SRC-8 sends
> that case to the duplicate report rather than to an import-time gate. What survives is the harder
> half. A path may depend on the item and on how it arrived, never on **when** it arrived or on
> **what a person later decided about it**. Import order is a property of a run rather than of an
> item, so an ordinal disambiguating suffix is still forbidden. A label assigned after import is
> supplied by no route, so a directory named for one is still forbidden, which also keeps the level
> from becoming a junk drawer for multi-valued groupings.
...
> path a function of the item's bytes, its route-supplied metadata, and its route, all of which are in
> hand at the moment of refusal. Writing the receipt where the item _would_ have gone puts the record
...
| ID          | Requirement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **R-CFG-1** | Turbo-Collection MUST read acquisition route declarations, the copies expected to exist, and every option from an external configuration file. It MUST NOT hardcode any path.                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| MUST NOT      | Delete or modify anything at the origin (R-SRC-7); transcode or strip metadata (R-SRC-5); split a multi-file item (R-SRC-9); let anything but the item, its route-supplied metadata, and its route decide a collection path (R-SRC-10); report or retain which items the source no longer supplies (R-SRC-13) |
...
assumed.** This blocks the route specifications, and it is recorded here as an open question rather than as
...
2. **Source viability.** Which iPhone route can actually satisfy R-SRC-5 (Section 12.2). This is
   empirical, and it blocks the iCloud route specification.
