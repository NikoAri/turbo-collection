# Turbo-Collection: Import Source Specification, iCloud
...
and can change it without notice, so every claim here is a dated observation with an expiry, never a
...
this document splits and each way becomes its own directory under `specs/import-sources/`.
...
- **Dependency:** none, or a vendor tool required to import, or a vendor tool required to read
